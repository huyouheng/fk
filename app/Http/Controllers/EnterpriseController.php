...
class EnterpriseController extends Controller
{
    public function detail(Request $request, $id) {
        $enterprise = Company::with(['report', 'files'])->findOrFail($id);
    ...
    public function audit(AuditPostRequest $request, $id) {
        try {
            ...
            if ($status == 2) {
                //TODO 表示通过
                if ($report->time_status == 1) {
                    // TODO 表示第一次提交
                    $attribute['time_status'] = 2;
                }
                if ($money){
                    Company::where('id', $id)->update(['money' => $money]);
                }
            }
            if (!empty($revision)) {
    ...
    public function revisions(Request $request) {
        if($enterprise_id = $request->get('enterprise_id')) {
            $enterprise = Company::with('report')->findOrFail($enterprise_id);
            ...
            $enterprise = Company::with('report')->findOrFail($request->user()->enterprise_id);
            $revisions = $enterprise->report->revisions ?? [];
            $towns = TownType::all()->pluck('TownName', 'TownID');
    ...
    public function index(Request $request)
    {
        if ($request->user()->is_admin) {
            ...
            if (!empty($request->user()->industry_id_min) && !empty($request->user()->industry_id_max)) {
                $model->whereBetween('id', [$request->user()->industry_id_min, $request->user()->industry_id_max]);
            }
            $industries = $model->where('pid', '!=', 0)->pluck('name', 'id');
            $towns = TownType::all()->pluck('TownName', 'TownID');

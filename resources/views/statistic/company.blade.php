        ...
        <div class="row query-row2" style="margin-top:5px;">
            <div class="col-md-12">
                ...
                <a href="/statistical/company?reportStatus={{session('reportStatus')}}&industry=0&townType={{session('townType')}}&EnterpriseName={{session('EnterpriseName')}}&Address={{session('Address')}}&start={{session('start')}}&end={{session('end')}}&per_page={{session('per_page')}}">
                    <button class="btn @if(!session('industry')) btn-primary @endif" data-id="">全部</button>
                </a>
                @foreach($industry as $key => $indust)
                <a href="/statistical/company?reportStatus={{session('reportStatus')}}&per_page={{session('per_page')}}&start={{session('start')}}&end={{session('end')}}&ctime={{time()}}&&industry={{$indust->name}}&townType={{session('townType')}}&EnterpriseName={{session('EnterpriseName')}}&Address={{session('Address')}}">
                    <button class="btn industryBtn @if(session('industry') == $indust->name) btn-primary @endif" data-id="{{$key}}">{{$indust->name}}</button>
                </a>
                @endforeach
            </div>
        </div>
        ...
        <table class="table-striped table-bordered table-hover" id="datatable">
            ...
            @if ($enterprises->count())
                @foreach($enterprises as $enterp)
                    <tr>
                        <td>{{$enterp->title}}</td>
                        <td>{{$enterp->sm_class}}</td>
                        <td>{{substr($enterp->start_at,0,10)}}</td>
                        <td>
                            @if(!$enterp->report)
